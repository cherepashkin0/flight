...
def setup_mlflow(tracking_uri, experiment_name):
    ...
    if parsed.scheme in ("", "file"):
        ...
        logger.info(f"Ensured MLflow directory exists at: {local_dir}")
...
# Refactored HyperparameterOptimizer with extracted helper for merging params
class HyperparameterOptimizer:
    """Handles hyperparameter optimization using Optuna with a generic suggestion method."""
    def __init__(self, X_train, y_train, model_type, config):
        ...
        # Setup search space and fixed params
    ...
    def _suggest_params(self, trial):
        """Suggest hyperparameters dynamically based on config-driven search_space."""
        params = {}
    ...
    def _merge_params(self, params_dict):
        """Merge suggested hyperparameters with fixed parameters."""
        return {**params_dict, **self.fixed_params}

    def _objective(self, trial):
        # Suggest and merge parameters
        ...
        categorical_cols = self.X_train.select_dtypes(exclude=['int64','float64']).columns.tolist()
        ...
        pipeline.named_steps['model'] = ModelFactory.apply_class_balancing(
            pipeline.named_steps['model'], self.model_type, class_ratio
        )
        ...
        cv_results = cross_validate(
            ...
            cv=self.cv, scoring=scorer, n_jobs=1, return_train_score=False
        )
    ...
    def optimize(self):
        ...
        # Merge best params and save
        best_params = self._merge_params(study.best_params)
    ...
    def train(self):
        ...
        try:
            ...
            with mlflow_run:
                ...
                # --- simply train on the full data with no callbacks or early stopping ---
                logger.info(f"Training {self.model_type} to completion…")
